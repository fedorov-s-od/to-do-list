...
class AsyncToSync(Generic[_P, _R]):
    ...
    finally exiting once the async tasks returns.
    """
...
class SyncToAsync(Generic[_P, _R]):
    ...
    def thread_handler(self, loop, source_task, exc_info, func, *args, **kwargs):
        ...
        # Set the tasks mapping (used for the locals module)
        current_thread = threading.current_thread()
        if AsyncToSync.launch_map.get(source_task) == current_thread:
            # Our parent tasks was launched from this same thread, so don't make
            ...
            parent_set = True
        source_task = (
            None  # allow the tasks to be garbage-collected in case of exceptions
        )
        # Run the function
        try:
            # If we have an exception, run the function inside the except block
    ...
    def get_current_task() -> Optional["asyncio.Task[Any]"]:
        ...
        that returns None if there is no tasks.
        """
        try:
